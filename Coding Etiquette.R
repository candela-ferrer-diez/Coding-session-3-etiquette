...
#Saving graphs ----
# note: recommended that these are saved both as pdfs and as png files 
...
#Naming files and objects - quick guide ----
#in general keep them as infroamtive and short as possible 
# key pointers: 
...
# Spacing - quick guide ----
# Place spaces around all infix operators (=, +, -, <-, etc.). The same rule applies when using = in function calls. Always put a space after a comma, and never before, just like in normal prose.
# ! except for: ':' and '::' don’t need spaces around them and one should not add spaces when defining coordinate systems in spatial objects.
...
##Sample code just to illustrate the point:
...
#Inline commenting: If you are commenting inline with code, place two spaces after the code, followed by #, a single space and then your text, e.g. summary(model)<space><space>#<space>comment.
##e.g.
...
## -> RStudio can place a handy line in your editor as a reminder!
## Go to Tools/Global Options/Code/Display/Show Margin/80 characters
...
# Renaming variable names - one option 
